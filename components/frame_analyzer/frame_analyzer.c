...
static void data_frame_handler(void *args, esp_event_base_t event_base, int32_t event_id, void *event_data) {
    ...
    if(search_type == SEARCH_HANDSHAKE){
        // Post with short timeout instead of portMAX_DELAY to avoid hanging the
        // sniffer task when the event queue is full (heavy deauth/attack traffic).
        esp_err_t post_err = esp_event_post(FRAME_ANALYZER_EVENTS,
                                            DATA_FRAME_EVENT_EAPOLKEY_FRAME,
                                            frame,
                                            sizeof(wifi_promiscuous_pkt_t) + frame->rx_ctrl.sig_len,
                                            pdMS_TO_TICKS(10));
        if(post_err != ESP_OK){
            // Silently drop the frame if the queue is full rather than blocking
            // the sniffer task. The handshake state machine will recover from
            // the next EAPoL frame that comes through.
            ESP_LOGV(TAG, "EAPoL event queue full, dropping frame");
        }
        return;
    }
