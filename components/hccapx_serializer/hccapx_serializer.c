...
static void ap_message_m3(eapol_packet_t* eapol_packet, eapol_key_packet_t *eapol_key_packet){
    ...
    if(eapol_source == 2){
        ...
        eapol_source = 3;  // mark M3 as processed so subsequent M4 can be handled
...
static void sta_message_m4(eapol_packet_t* eapol_packet, eapol_key_packet_t *eapol_key_packet){
    ...
    if((message_sta == 2) && (eapol_source != 0)){
        if(eapol_source == 3){
            // M3 already processed — M4 completes the 4-way handshake
            hccapx.message_pair = 4;
            ESP_LOGD(TAG, "Complete 4-way: M2+M3 -> M4, mp=4");
            return;
        }
        // Already have M2 from another STA or retransmit — skip
