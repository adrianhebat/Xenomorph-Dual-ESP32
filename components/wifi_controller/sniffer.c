...
void wifictl_sniffer_start(uint8_t channel) {
    ...
    // ESP32 cannot switch port if some STA is connected to AP.
    // Best-effort kick — fails silently when AP interface is disabled
    // (e.g. after the join attack switched to STA-only mode).
    ESP_LOGD(TAG, "Kicking all connected STAs from AP (best-effort)");
    esp_err_t deauth_ret = esp_wifi_deauth_sta(0);
    if (deauth_ret != ESP_OK) {
        ESP_LOGD(TAG, "esp_wifi_deauth_sta returned 0x%x (ignored)",
                 deauth_ret);
    }
