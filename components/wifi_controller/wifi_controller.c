...
void wifictl_ap_start(wifi_config_t *wifi_config) {
    ...
    // If we're in STA-only mode (e.g. after joining a target network),
    // the AP interface is disabled and esp_wifi_set_config(AP) will fail.
    // Switch to APSTA first so the AP can come back up.
    wifi_mode_t current_mode;
    if (esp_wifi_get_mode(&current_mode) == ESP_OK &&
        current_mode == WIFI_MODE_STA) {
        ESP_LOGW(TAG, "Mode is STA-only — switching to APSTA to bring AP up");
        esp_err_t mr = esp_wifi_set_mode(WIFI_MODE_APSTA);
        if (mr != ESP_OK) {
            ESP_LOGE(TAG, "Failed to switch to APSTA: 0x%x", mr);
        }
    }
...
void wifictl_restore_ap_mac(){
    ...
    // Best-effort — fails when AP interface is disabled (e.g. STA-only mode).
    esp_err_t ret = esp_wifi_set_mac(WIFI_IF_AP, original_mac_ap);
    if (ret != ESP_OK) {
        ESP_LOGD(TAG, "esp_wifi_set_mac(AP) returned 0x%x (ignored — AP may be down)", ret);
    }
}
