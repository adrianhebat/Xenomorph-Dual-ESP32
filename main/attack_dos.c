...
void attack_dos_start(attack_config_t *attack_config) {
    ...
    switch(method){
        ...
        case ATTACK_DOS_METHOD_COMBINE_ALL:
            ESP_LOGD(TAG, "ATTACK_DOS_METHOD_COMBINE_ALL (aggressive multi-frame)");
            attack_method_rogueap(attack_config->ap_record);
            // Changed from 1s to 0.1s period → 10 ticks/sec × 3 frames/tick = 30 frames/sec
            // ESP_LOGD removed from wsl_bypasser to prevent UART overload and WDT crash.
            attack_method_broadcast(attack_config->ap_record, 0);
            break;
        default:
