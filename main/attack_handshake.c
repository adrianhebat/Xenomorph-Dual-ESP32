...
static void eapolkey_frame_handler(void *args, esp_event_base_t event_base, int32_t event_id, void *event_data) {
    // Skip if handshake already captured — prevents spam after mp=4
    if (handshake_done) {
        return;
    }
    ...
    // Check if handshake is now complete (mp=4 = full 4-way handshake)
    hccapx_t *hc = hccapx_serializer_get();
    if (hc && hc->message_pair == 4) {
        handshake_done = true;
        ESP_LOGI(TAG, "Handshake complete (mp=4) — stopping handler");
    }
}
...
void attack_handshake_stop(){
    if (method == -1) {
        ESP_LOGD(TAG, "attack_handshake_stop: already stopped, skipping");
        return;
    }
    switch(method){
        case ATTACK_HANDSHAKE_METHOD_BROADCAST:
    ...
    esp_event_handler_unregister(ESP_EVENT_ANY_BASE, ESP_EVENT_ANY_ID, &eapolkey_frame_handler);
    ap_record = NULL;
    method = -1;
